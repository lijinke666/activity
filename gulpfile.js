...
gulp.task('buildimage',()=>{
  gulp.src(pathBuild("src/images/*.{png,jpg,gif,jpeg}"))
      ...
      .pipe(gulp.dest(pathBuild('src/build/images')))
})

//开发服务器
gulp.task('server',['buildcss','buildjs','buildimage'],()=>{
  browserSync.init({
    server:{
      baseDir:"./src"
    }
  ...
  gulp.watch(pathBuild('src/js/*.js'),['buildjs']);
  gulp.watch(pathBuild('images/*.{png,jpg,gif,jpeg}'),['buildimage']);
